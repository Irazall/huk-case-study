...
data_sev_agg <- data_sev %>% group_by(IDpol) %>% summarise(ClaimAmount = sum(ClaimAmount), ClaimNb_sev = n())
...
```{r quantiles}
quantile(data_sev_agg_clinfo$ClaimAmount, probs = c(0, 0.005, 0.01, 0.025, 0.05, 0.95, 0.975, 0.99, 0.995, 1))
# winsorize 0.5 % from above and below
data_sev_agg_clinfo$ClaimAmount <- Winsorize(data_sev_agg_clinfo$ClaimAmount, val = quantile(data_sev_agg_clinfo$ClaimAmount, probs = c(0.005, 0.995), na.rm = FALSE))
```

We winsorize only 1 % of the claims, i.e., half a percent from above and half a percent from below. Thereby, we follow the recommendation by the DAV based on the Tschebychev inequality.
...
for (i in c("VehPower", "VehAge", "DrivAge", "BonusMalus", "Density")) {
  ...
  # winsorize 0.5 % from above and below
  data_sev_agg_clinfo[[i]] <- Winsorize(data_sev_agg_clinfo[[i]], val = quantile(data_sev_agg_clinfo[[i]], probs = c(0.005, 0.995), na.rm = FALSE))
}
```
